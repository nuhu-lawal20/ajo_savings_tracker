...
export default async function PublicHowItWorksPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7fb] dark:bg-[#071322] text-slate-900 dark:text-white flex flex-col antialiased">
      {/* ── TOP PUBLIC HEADER ── */}
      <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/80 dark:bg-[#071322]/80 border-b border-border/60 px-4 sm:px-8 py-3.5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <KadasheLogo withLink size="md" />
        </div>

        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/">
            <Button variant="ghost" size="sm" className="text-xs font-bold text-muted-foreground hover:text-foreground cursor-pointer">
          ...
          {user ? (
            <Link href="/dashboard">
              <Button size="sm" className="rounded-full bg-[#0F2744] hover:bg-[#0A1C33] text-white font-bold text-xs px-4 cursor-pointer shadow-sm">
                <LayoutDashboard className="mr-1.5 h-3.5 w-3.5 text-sky-400" />
                Go to Dashboard
              </Button>
            </Link>
          ) : (
            <Link href="/login">
              <Button size="sm" className="rounded-full bg-[#0284C7] hover:bg-[#0369A1] text-white font-bold text-xs px-4 cursor-pointer shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Card className="rounded-3xl border border-[#e1e8f0] dark:border-sky-500/20 bg-white dark:bg-[#071322] shadow-sm p-5 space-y-2.5">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              In traditional Adashe, the organizer often unfairly assigns themselves Turn #1. On Kadashe, <strong>the organizer can NEVER receive Turn #1</strong>. Turn #1 is guaranteed to the highest-trust peer member, and remaining turns are mathematically sorted by AI Trust Score.
            </p>
          </Card>
